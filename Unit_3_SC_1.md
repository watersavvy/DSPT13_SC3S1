...
We recommend, for the purposes of this Sprint Challenge, to complete the files on the
same level in your directory (and not use a subdirectory for your test files).
...
This file is Markdown, so it can be helpful to render with PyCharm, VS Code or a
...
- `identifier` (integer, automatically generated as a random number
...
- `stealability(self)` - calculates the price divided by the weight, and then
  _returns_ a message: if the ratio is less than 0.5 return "Not so stealable...",
...
- `explode(self)` - calculates the flammability times the weight, and then
  _returns_ a message: if the product is less than 10 return "...fizzle.", if it is
...
- `flammability` should bea uniform float from 0.0 to 2.5
